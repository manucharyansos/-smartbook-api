...
class DemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $starterSalon = Business::updateOrCreate(
            ...
            [
                ...
                'vertical' => 'services',
                'business_category_id' => $this->categoryId('beauty-salon'),
                'is_public' => true,
                'is_verified' => true,
                'phone' => '+37491000001',
        ...
        $proClinic = Business::updateOrCreate(
            ...
            [
                ...
                'vertical' => 'healthcare',
                'business_category_id' => $this->categoryId('dental-clinic'),
                'is_public' => true,
                'is_verified' => true,
                'phone' => '+37491000004',
                'address' => 'Yerevan, Armenia',
        ...
        foreach ([$starterSalon, $proSalon, $proClinic, $businessClinic] as $business) {
            $this->ensurePrimaryLocation($business);
        }
        ...
        // =========================
        $this->createSubscriptions($starterSalon, 'start');
        $this->createSubscriptions($proSalon, 'studio');
        $this->createSubscriptions($proClinic, 'studio');
        $this->createSubscriptions($businessClinic, 'scale');
    ...
    {
        return BusinessCategory::query()->where('slug', $slug)->value('id');
    }

    private function ensurePrimaryLocation(Business $business): void
    {
        $coordinates = [
            'starter-salon-yerevan' => [40.1848, 44.5121, 'Կենտրոն, Սարյան փողոց'],
            'pro-salon-yerevan' => [40.2056, 44.5031, 'Արաբկիր, Կոմիտաս պողոտա'],
            'pro-clinic-yerevan' => [40.1789, 44.5152, 'Կենտրոն, Մաշտոցի պողոտա'],
            'business-clinic-yerevan' => [40.1976, 44.4897, 'Դավթաշեն, Տիգրան Պետրոսյան փողոց'],
        ];

        [$latitude, $longitude, $address] = $coordinates[$business->slug] ?? [40.1772, 44.5035, $business->address ?: 'Yerevan, Armenia'];

        $business->locations()->updateOrCreate(
            ['is_primary' => true],
            [
                'name' => $business->name,
                'address' => $address,
                'city' => 'Yerevan',
                'district' => explode(',', $address)[0] ?? null,
                'latitude' => $latitude,
                'longitude' => $longitude,
    ...
    private function primaryLocationId(Business $business): ?int
    {
        return $business->locations()->where('is_primary', true)->value('id')
            ?: $business->locations()->orderBy('sort_order')->orderBy('id')->value('id');
    }
    ...
    private function createBusinessUsers(Business $business, string $prefix, bool $isClinic = false): void
    {
        ...
        User::updateOrCreate(
            ...
            [
                ...
                'location_id' => $this->primaryLocationId($business),
                'is_active' => true,
            ]
        );
    ...
    private function createBeautyServices(Business $business): array
    {
        ...
        foreach ($services as $serviceData) {
            ...
            if ($locationId = $this->primaryLocationId($business)) {
                $service->locations()->syncWithoutDetaching([$locationId]);
            }
            $created[] = $service;
        }
    ...
    private function createSubscriptions(Business $business, string $planCode = 'studio'): void
    {
